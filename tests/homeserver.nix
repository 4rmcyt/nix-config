...
let
  homeserver = import ../hosts/nixos/homeserver {
    ...
    # config = { my.security.ssl.certPath = "/var/lib/acme/labhome.work/fullchain.pem"; };
  };
in
pkgs.nixosTest {
  ...
  testScript = ''
    ...
    machine.succeed("curl --fail http://home.labhome.work | grep 'Homepage'");
    machine.succeed("curl --fail http://paperless.labhome.work | grep 'Paperless-ngx'");
    machine.succeed("curl --fail http://hass.labhome.work | grep 'Home Assistant'");
