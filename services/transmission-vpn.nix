  ...
  services = {
    ...
    transmission = {
      ...
      settings = {
        ...
        rpc-enabled = true;
        # rpc-bind-address will be set by the start script
        rpc-whitelist = "127.0.0.1,192.168.1.*,100.64.0.*,localhost,transmission.example.com";
        rpc-whitelist-enabled = true; # Strongly recommend enabling this for security
        dht-enabled = true; # Should be boolean, not string "true"
  ...
  systemd.services.transmission = {
    ...
    serviceConfig = {
      ExecStart = mkForce ''
        ${startTransmission}
      '';
      # Add network namespace isolation for stronger binding
      # Note: This is an advanced technique and might require more configuration
      # if you need other services to access Transmission via localhost etc.
      # You'd also need to ensure the `transmission` user has permissions for `ip`
      # and `netns` if you go this route.
      # IPAddressDeny = "any"; # This is for Systemd-networkd, not direct binding
      # BindToDevice = piaInterface; # This is more for networkd, or a simpler Systemd service.
      # PrivateNetwork = true; # Isolates the service to its own network namespace
      # IPAddress = [ "$IP/32" ]; # If PrivateNetwork is true, assign this IP within the namespace.
      # The simpler approach without PrivateNetwork:
      # Network Namespace setup for better isolation (advanced, but effective)
      # This needs more careful configuration than just a few lines.
      # For now, let's stick to simply routing through the VPN.
    };
  };
}