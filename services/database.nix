...

{
  ...
  systemd.services.postgresql-setup-passwords = {
    ...
    script = ''
      ${pkgs.postgresql_15}/bin/psql -c "ALTER USER keycloak WITH PASSWORD '${config.sops.secrets.database_passwords.keycloak_db_password.pass}';"
      ${pkgs.postgresql_15}/bin/psql -c "ALTER USER nextcloud WITH PASSWORD '${config.sops.secrets.database_passwords.nextcloud_db_password.pass}';"
