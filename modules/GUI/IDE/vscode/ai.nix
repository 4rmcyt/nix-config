{
  ...
  claudeMd = pkgs.writeText "CLAUDE.md" (systemPrompts.claude lib config.programs.mcp.servers);
in {
  ...
  home.activation.claudeDesktopConfig = lib.hm.dag.entryAfter ["writeBoundary"] ''
    ...
    mkdir -p "$HOME/.config/VSCodium/User/globalStorage/saoudrizwan.claude-dev/settings"
    mkdir -p "$HOME/.config/claude"

    cp -f ${mcpConfigJson} "$HOME/.config/Claude/claude_desktop_config.json"
    cp -f ${mcpConfigJson} "$HOME/.config/VSCodium/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json"
    cp -f ${claudeMd} "$HOME/.config/claude/CLAUDE.md"

    chmod 600 "$HOME/.config/Claude/claude_desktop_config.json"
    chmod 600 "$HOME/.config/VSCodium/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json"
