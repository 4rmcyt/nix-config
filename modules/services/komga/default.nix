{
  ...
  systemd.services.komga.serviceConfig = {
    ...
    # BISECT 2026-09-15, round 0: baseline (just UMask/BindPaths/Environment)
    # — the full shh-generated hardening block below (mount/kernel
    # protection + CapabilityBoundingSet + RestrictAddressFamilies incl.
    # AF_NETLINK + SocketBindDeny + SystemCallFilter) still failed the same
    # "Operation not permitted" / Net.bind0 bind error even after restoring
    # AF_NETLINK, and diffs clean against the last confirmed-working commit
    # (0f2ddf01) — so the single-directive theory is exhausted. Reverting to
    # bare baseline to confirm it still starts, then re-adding in groups
    # (mount/kernel-protection+Capability, then network, then syscall) with
    # a rebuild+test between each, instead of guessing further. Full block
    # preserved in git history (this file, commit before this one).
  };
