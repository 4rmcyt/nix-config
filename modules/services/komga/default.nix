{
  ...
  systemd.services.komga.serviceConfig = {
    ...
    # rebuild that komga still serves on its port: SocketBindDeny includes
    # "ipv4:tcp", which is unusual for a web server — likely fine because
    # the JVM binds dual-stack on "::" (AF_INET6) rather than a raw AF_INET
    # socket, so shh never observed a real ipv4:tcp bind to allow, but this
    # is the first thing to suspect if the port stops listening.
    ProtectSystem = "full";
    ProtectHome = true;
