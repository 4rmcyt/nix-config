...
in {
  options.my.defaults = {
    ...
    gateway = lib.mkOption {
      type = lib.types.str;
      default = lib.removeSuffix "\n" (builtins.readFile config.sops.secrets.defaults-gateway.path);
      description = "Default gateway IP address";
    };

    homeserver_lan = lib.mkOption {
      type = lib.types.str;
      default = lib.removeSuffix "\n" (builtins.readFile config.sops.secrets.defaults-homeserver_lan.path);
      description = "Local IP address of the homeserver";
    };

    desktop_lan = lib.mkOption {
      type = lib.types.str;
      default = lib.removeSuffix "\n" (builtins.readFile config.sops.secrets.defaults-desktop_lan.path);
      description = "Local IP address of the desktop LAN connection";
    };

    desktop_wifi = lib.mkOption {
      type = lib.types.str;
      default = lib.removeSuffix "\n" (builtins.readFile config.sops.secrets.defaults-desktop_wifi.path);
      description = "Local IP address of the desktop WiFi connection";
    };

    matebook_wifi = lib.mkOption {
      type = lib.types.str;
      default = lib.removeSuffix "\n" (builtins.readFile config.sops.secrets.defaults-matebook_wifi.path);
