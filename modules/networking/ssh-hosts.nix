{config, ...}: {
  ...
  # SSH Configuration
  # =================================================================
  # Common SSH client configuration for all hosts
  # Written directly to avoid home-manager symlink permission issues
  system.activationScripts.sshConfig = ''
        mkdir -p /home/${config.my.defaults.user}/.ssh
        cat > /home/${config.my.defaults.user}/.ssh/config << 'EOF'
    Host *
      AddKeysToAgent yes
      ControlMaster auto
      ControlPersist 10m
    ...
    Host homeserver
      ...
      IdentityFile ~/.ssh/${config.my.defaults.user}
      IdentitiesOnly yes
    ...
    Host github.com
      ...
      IdentitiesOnly yes
    EOF
        chown ${config.my.defaults.user}:users /home/${config.my.defaults.user}/.ssh/config
        chmod 600 /home/${config.my.defaults.user}/.ssh/config
  '';

  # =================================================================
  # /etc/hosts Configuration
  # =================================================================
  # Common host entries for all machines
  networking.hosts = {
    # Gateway / Router
  ...
  # SSH Known Hosts
  # =================================================================
  # Common SSH known hosts for secure connections
