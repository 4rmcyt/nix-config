...
| `build-and-check-systems` | needs `discover-hosts` (and transitively `validate` + `security-checks`) | `reusable-build.yml` — matrix build over `discover-hosts`'s output. Currently `homeserver`, `gcp-relay` (`matebook` excluded — decommissioned). |
...
**Host matrix:** derived automatically by the `discover-hosts` job from `parts/hosts/*/` — a newly added host is built with no workflow edit. Only the `EXCLUDE` list is hand-maintained, in that job's `List CI-buildable hosts` step. Currently `desktop` and `matebook`: `desktop`'s full GUI closure (mango + the whole desktop stack) regularly exceeds GitHub-hosted runner disk/RAM and kills the runner outright — no build error, no log output, job just dies (built locally on that machine instead, `just deploy-local` / `nh os switch` — see "user builds himself" in CLAUDE.md); `matebook` is decommissioned and no longer deployed.
